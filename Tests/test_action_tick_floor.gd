	...
	for id in ActionLibrary.all_ids():
		...
		for label in ["wind-up", "recover"]:
			...
			if base <= 0:
				continue
			checked += 1
			var scaled := pawn.scale_action_ticks(base)
...
func test_the_floor_actually_fires_at_the_shipped_cap() -> void:
	var pawn := _pawn(&"warrior", ABSURD_AGI)
	var scale: float = PawnData.MAX_AGI_TICK_SCALE
	var floored := []
	for id in ActionLibrary.all_ids():
		var a := ActionLibrary.get_action(id)
		if a == null:
		...
		for base in [a.wind_up_ticks, a.recover_ticks]:
			...
			if int(round(float(base) * (1.0 - scale))) == 0:
				floored.append(id)
				assert_eq(pawn.scale_action_ticks(base), 1,
...
func test_no_enemy_action_can_be_scaled_below_one_tick() -> void:
	assert_eq(MonsterProfile.scale_action_ticks(1, 1000.0), 1,
		"a one-tick enemy action at an absurd action speed")
	assert_eq(MonsterProfile.scale_action_ticks(3, 1000.0), 2,
...
func test_an_ordinary_pawn_is_not_touched_by_the_floor() -> void:
	...
	for cid in ClassLibrary.all_ids():
		...
		var scale: float = clampf(
			pawn.effective_attribute(CG.Attribute.AGI) * PawnData.AGI_TICK_SCALE_PER_POINT,
			0.0, PawnData.MAX_AGI_TICK_SCALE)
		for base in [3, 8, 20, 45, 90]:
			var expected := int(round(float(base) * (1.0 - scale)))
			assert_eq(pawn.scale_action_ticks(base), expected,
...
func test_a_zero_tick_action_stays_zero() -> void:
	...
	assert_eq(pawn.scale_action_ticks(0), 0, "a zero-tick action must not be given a tick")
	assert_eq(MonsterProfile.scale_action_ticks(0, 2.0), 0, "the enemy path, same rule")
