...
func test_exactly_two_actions_leave_a_pool_and_neither_is_the_fire_spell() -> void:
	...
	for id in ActionLibrary.all_ids():
		if ActionLibrary.get_action(id).leaves_pool_radius > 0.0:
			with_pool.append(id)
	...
	assert_eq(ActionLibrary.get_action(&"geyser_scald").damage_type, CG.DamageType.FIRE,
...
func test_the_pool_is_on_the_action_a_pawn_nobody_configured_will_cast() -> void:
	...
	for id in ActionLibrary.actions_for_pawn(pawn):
		actions.append(ActionLibrary.get_action(id))
