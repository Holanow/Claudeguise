...
## EnemyLibrary.all_ids(): the missing fourth sibling of all_class_ids/
...
func test_all_enemy_ids_is_not_empty() -> void:
	assert_true(EnemyLibrary.all_ids().size() > 0, "expected at least one registered enemy")
...
func test_all_enemy_ids_is_not_derived_from_encounters() -> void:
	var enemy_ids := EnemyLibrary.all_ids()
