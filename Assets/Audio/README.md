...
| `event/loot_awarded.ogg` | Issue 811: a pawn puts on something the last room dropped, announced on the first tick of the next room. Roughly one and a half a floor, so it is rare enough to earn a real sound and this is the best candidate in the table. Silent only because nobody has recorded one. |
...
| `event/leeched.ogg` | Issue 931: a leech affix returns a share of a hit to the attacker. Silent because it arrives in the same tick as the `damage` that caused it, which already makes a noise. |
| `event/resource_gained.ogg` | Issue 931: a kill refunds a share of the killer's own pool. Silent because it arrives in the same tick as the `death` that caused it. |
