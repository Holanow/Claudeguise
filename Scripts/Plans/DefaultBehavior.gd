...
##
## Not a duplicate of `CombatSim._decide_phase`'s compulsion: that reads
## TAUNTED, stamped on everyone inside the radius at the moment of the cast,
## while this reads TAUNTING, which sits on the taunter for the duration. A
## unit that walks into the radius after the cast is never TAUNTED, so this is
## the only thing that makes it prefer the taunter. See #302.
##
## Deterministic: iterates the same `enemies` array `_nearest` does, first
## strictly-closer candidate wins.
