...
## Issue 764: the Siege Master's "Mark the target" row on the screen the player
## reads it on, so the new refusal -- "an enemy within 350 units has no Marked"
...
func _ready() -> void:
	...
	await _run()
	print("MarkRowShot: %d failure(s)" % _failures)
...
func _run() -> void:
	...
	for n in _walk(_main):
		if n.get_script() != null and n.get_script().resource_path.ends_with("PartySelect.gd"):
			select = n
	if select == null:
		printerr("MarkRowShot: no party screen")
		_failures += 1
		return
	...
	for p in select.available_pawns():
		if p.pawn_class != null and p.pawn_class.id == &"siege_master":
			pawn = p
	if pawn == null:
		printerr("MarkRowShot: no Siege Master on the party screen")
		_failures += 1
		return
	...
	if not select._cards.has(pawn.id):
		printerr("MarkRowShot: no Siege Master card")
		_failures += 1
		return
	...
	for c in _texts(panel):
		var t: String = c.text
		if t.to_lower().contains("has no marked"):
			print("MarkRowShot: '%s'" % t)
			if wanted == null:
				wanted = c
	if wanted == null:
		printerr("MarkRowShot: the library never says the Mark row refuses a marked enemy")
		_failures += 1
		return
	...
	get_viewport().get_texture().get_image().save_png("%s/curlew6_764_mark_row_refuses_a_second_mark.png" % OUT_DIR)
	print("MarkRowShot: curlew6_764_mark_row_refuses_a_second_mark.png")
