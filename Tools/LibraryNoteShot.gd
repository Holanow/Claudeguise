...
func _run() -> void:
	...
	for p in select.available_pawns():
		if p.pawn_class != null and p.pawn_class.id == &"geysermancer":
			pawn = p
	...
	for n in _walk(panel):
		if n is Label and n.text.begins_with(InspectPanel.LIBRARY_NEEDS_MARK):
			rows.append(n)
	...
	get_viewport().get_texture().get_image().save_png("%s/issue434_library_%s.png" % [OUT_DIR, _tag])
	print("FallbackRowProbe: wrote issue434_library_%s.png" % _tag)
